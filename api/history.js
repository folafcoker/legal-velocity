...
// Business-day helpers (mirrors the logic in index.html)
...
module.exports = async function handler(req, res) {
  ...
  // Fetch from sorted set (score = event timestamp ms, oldest → newest)
  const raw = await kv.zrange('juro:event_log', fromMs, toMs, { byScore: true });
  ...
  const events = raw
    ...
    .map(e => ({
      ...e,
      // Annotate with business-day duration for closed turns
      businessDays: e.sentToElaine && e.returnedDate
        ? businessDaysBetween(e.sentToElaine, e.returnedDate)
        : null,
    }))
    // Reverse so most-recent events come first
    .reverse();

  // Summary stats
  const closed   = events.filter(e => e.returnedDate);
